  ...
  const PEER = 'PersonaPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->seccion = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->estado = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->correo = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->segundonombre = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->segundoapellido = ($row[$startcol + 10] !== null) ? (string) $row[$startcol + 10] : null;
			$this->resetModified();
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[6] => $this->getSeccion(),
			$keys[7] => $this->getEstado(),
			$keys[8] => $this->getCorreo(),
			$keys[9] => $this->getSegundonombre(),
			$keys[10] => $this->getSegundoapellido(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[6], $arr)) $this->setSeccion($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setEstado($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setCorreo($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setSegundonombre($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setSegundoapellido($arr[$keys[10]]);
	}
